...
const Treatments = () => {
  const [featured, ...rest] = treatmentsPosts;
  const secondary = rest.slice(0, 3);
  const tags = Array.from(
    new Set(treatmentsPosts.flatMap((post) => post.tags))
  ).slice(0, 10);
      ...
      <main className="container mx-auto px-4 pt-28 pb-12 max-w-6xl">
        <header className="mb-8 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">
              Evidence‑informed Paths to Healing
            </h1>
            <p className="mt-2 text-muted-foreground max-w-2xl text-sm sm:text-base">
              Explore gentle explainer articles on therapies like DBT, CBT,
              psychoeducation, art therapy and more—written for real people,
              not textbooks.
            </p>
          </div>
          <Link
        ...
        <section className="grid gap-6 lg:grid-cols-[2fr,1.2fr]">
          ...
          <div className="space-y-6">
            {/* Featured card */}
            {featured && (
              <Link to={`/treatments/${featured.slug}`}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {secondary.map((post) => (
                <Link key={post.slug} to={`/treatments/${post.slug}`}>
          ...
          <aside className="space-y-4">
            ...
            <Card className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold">All treatments</h3>
                <span className="text-[11px] text-muted-foreground">
                  {treatmentsPosts.length} articles
                </span>
              </div>
              <div className="space-y-2">
                {treatmentsPosts.map((post) => (
                  <Link
                    key={post.slug}
                    to={`/treatments/${post.slug}`}
